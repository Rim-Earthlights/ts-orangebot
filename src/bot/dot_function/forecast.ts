 ...
 * @param onecall
 */
async function weatherTodayJson(forecast: Forecast, onecall: Onecall, cityName: string): Promise<{ name: string; value: string; }[]>{
    // 気温や気象情報
    const weather = forecast.weather[0].description;
    const cloud = forecast.clouds.all;
    const temp = forecast.main.temp.toFixed(1);
 ...
 * @returns
 */
async function weatherDayJson(onecall: Onecall, index: number, cityName: string): Promise<{ name: string; value: string; }[]>{
    // 気温や気象情報
    const weather = onecall.daily[index].weather[0].description;
    const cloud = onecall.daily[index].clouds;
    const temp = onecall.daily[index].temp.day.toFixed(1);
