...
/**
 * ユーザーがDBに登録済みか (= 利用規約に同意済みか) を確認する
 * @param gid サーバーID
 * @param uid ユーザーID
 * @returns 登録済みなら true
 */
export const isRegisteredUser = async (gid: string, uid: string): Promise<boolean> => {
  const user = await new UsersRepository().get(gid, uid);
  return user != null;
};
